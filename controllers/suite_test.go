...
var _ = BeforeSuite(func() {
	...
	err = connectivityv1alpha1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	// Namespace that holds the ddns-updater-config ConfigMap the controller
	// assembles into.
	Expect(k8sClient.Create(ctx, &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{Name: "ddns-updater"},
	})).To(Succeed())
	...
	err = (&DDNSRecordReconciler{
		Client:   k8sManager.GetClient(),
		Log:      ctrl.Log.WithName("controllers").WithName("DDNSRecord"),
		Scheme:   k8sManager.GetScheme(),
		Recorder: k8sManager.GetEventRecorderFor("ddnsrecord-controller"),
